...
/*Disconnect DataDesignLeadUser, Login as system to drop users and create other objects*/
...
/*Create DBA User and grant necessary permissions*/
...
/*Disconnect from System and connect to DBAUser*/
...
GRANT UNLIMITED TABLESPACE TO datadesignleaduser;
...
/*---------------------------------------------------*/
/*     CREATE TABLESPACE AND ASSIGN TO USERS         */
/*---------------------------------------------------*/

CREATE TABLESPACE "USER_DATA1" LOGGING DATAFILE '/u01/app/oracle/oradata/DBST670/user_data101.dbf' SIZE 3004K REUSE AUTOEXTEND ON NEXT 512K MAXSIZE 65536K EXTENT MANAGEMENT LOCAL;  
CREATE TABLESPACE "USER_DATA2" LOGGING DATAFILE '/u01/app/oracle/oradata/DBST670/user_data201.dbf' SIZE 2408K REUSE AUTOEXTEND ON NEXT 512K MAXSIZE 65536K EXTENT MANAGEMENT LOCAL;  
...
/*Disconnect from DBAUser, Login as DataDesignLeadUser*/
SELECT 'alter system kill session '''
       || sid
       || ','
       || serial#
...
CREATE TABLE zip (
    ...
    PCTFREE 5 PCTUSED 90 TABLESPACE user_data1
        STORAGE ( INITIAL 1092 k NEXT 200 k MAXEXTENTS UNLIMITED PCTINCREASE 0 ),
    PARTITION zip_part2
...
/*Analyze with deprecated command*/
...
ANALYZE TABLE product_cat COMPUTE STATISTICS;
ANALYZE TABLE region COMPUTE STATISTICS;
ANALYZE TABLE stores COMPUTE STATISTICS;
...
WHERE table_name ='CUSTOMER' 
        ...
        OR table_name = 'ORDER_ITEM'
        OR table_name = 'ORDERS'
        OR table_name = 'PAYMENT'
        OR table_name = 'PRODUCT'
        OR table_name = 'PRODUCT_CAT'
        OR table_name = 'REGION'
        OR table_name = 'STORES'
        OR table_name = 'ZIP';


/*Remove gathered statistics*/

ANALYZE TABLE customer DELETE STATISTICS;
ANALYZE TABLE employee DELETE STATISTICS;
ANALYZE TABLE order_item DELETE STATISTICS;
ANALYZE TABLE orders DELETE STATISTICS;
ANALYZE TABLE payment DELETE STATISTICS;
ANALYZE TABLE product DELETE STATISTICS;
ANALYZE TABLE product_cat DELETE STATISTICS;
ANALYZE TABLE region DELETE STATISTICS;
ANALYZE TABLE stores DELETE STATISTICS;
ANALYZE TABLE zip DELETE STATISTICS;

/*Run DBMS_STATS to get all stats*/

EXEC DBMS_STATS.gather_schema_stats(ownname => 'DATADESIGNLEADUSER');
...
/*Show valid status from all objects*/
SELECT
        substr
    (object_name,1,20),
...
/*Query row numbers from catalog*/
